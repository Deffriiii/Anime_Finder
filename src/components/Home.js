...
import '../CSS/Home.css'; // Pastikan untuk mengimpor file CSS
...
function Home() {
    ...
    const [filteredAnime, setFilteredAnime] = useState([]); // State untuk anime yang sudah difilter
    ...
    useEffect(() => {
        const fetchAnime = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_JIKAN_API_URL_ANIME}`);
                setAnime(response.data.data);
                setFilteredAnime(response.data.data); // Menyimpan data anime yang diambil
                setLoading(false);
            } catch (error) {
                setError(error);
                setLoading(false);
    ...
    const handleSearch = () => {
        // Filter anime berdasarkan query pencarian
        const filtered = anime.filter(animeItem =>
            animeItem.title.toLowerCase().includes(query.toLowerCase())
        );
...
function AnimeCard({ animeItem }) {
    ...
    const handleViewDetails = () => {
        navigate('/lihat-detail', { state: { animeItem } });
    };

    const truncateSynopsis = (synopsis, wordLimit) => {
        const words = synopsis.split(' ');
        return words.length > wordLimit ? words.slice(0, wordLimit).join(' ') + '...' : synopsis;
    };
    ...
    return (
        <li className="anime-card">
            ...
            <p><strong>Duration:</strong> {animeItem.duration}</p>
            <p>
                {truncateSynopsis(animeItem.synopsis, 10)}
            </p>
