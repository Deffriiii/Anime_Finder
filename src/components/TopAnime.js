...
function TopAnime() {
  ...
  const [filteredAnime, setFilteredAnime] = useState([]); // State untuk anime setelah pencarian
  ...
  useEffect(() => {
    const fetchAnime = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_JIKAN_API_URL}`);
        setAnime(response.data.data);
        setFilteredAnime(response.data.data); // Set filteredAnime dengan data awal
        setLoading(false);
      } catch (error) {
        setError(error);
        setLoading(false);
...
function AnimeCard({ animeItem }) {
  ...
  return (
    <li className="anime-card">
      ...
      <p>{truncateSynopsis(animeItem.synopsis, 10)}</p>
